...
> **補足**
> 本ドキュメントは、2025-12-30 時点の初期版構成で実施した結合テスト結果の記録である。
> 現在の構成・テスト状況とは一部異なる可能性があるため、最新の構成、テスト方針、CI、自動テストの詳細は README、基本設計書、詳細設計書を参照する。
...
- 本ドキュメントでは、自動テストとは別に、  
  **画面操作を通した手動結合テスト**の結果を記録する
- 自動テストでは確認しきれない、画面遷移・認証状態・権限分離・本番相当環境での疎通を確認する
- 実務で想定される操作フローを再現
...
以上の結果より、本結合テスト範囲において、  
本番相当環境での画面表示、API連携、認証、権限分離に重大な問題がないことを確認した。
