...
現時点では、業務上の重要度が高いバックエンドの一部ロジックについて
単体テストを実装しています。
- 入金割当（Payment Allocation）更新処理
- 請求ステータス（未入金／一部入金／入金済み／期限超過）の再計算ロジック

これらは GitHub Actions 上で自動実行され、
リファクタリングや仕様変更時の安全性を確保しています。

今後の改善として、以下の段階的なテスト導入を想定しています。

### バックエンド
- ドメインロジックの単体テストの拡充
  - 請求ステータス自動判定ロジック（網羅ケースの追加）
  - 入金割当（Payment Allocation）計算処理の境界値テスト
- API 単体テスト（認証・権限制御含む）
